...
- Voice transcript turns derive owner status from `commands.ownerAllowFrom` when configured; otherwise they fall back to Discord `allowFrom` (or `dm.allowFrom`). Non-owner speakers cannot access owner-only tools (for example `gateway` and `cron`).
