...
- Gateway/channels: re-read config after channel stop (and after CLI plugin resolve) before logout persist, so concurrent MCP/channel/plugin writes are not wiped when Telegram/LINE/Nextcloud token clears land.
