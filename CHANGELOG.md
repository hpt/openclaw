...
- Plugins/config: re-read config after plugin/hook install and update I/O before writing so `createMergePatch` cannot wipe concurrent channel/MCP/plugin keys from a stale full-config snapshot.
