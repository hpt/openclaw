...
- Config/on-demand channel install: persist `channels resolve`, `channels capabilities`, `directory`, and `channels login|logout` plugin installs from a fresh snapshot so concurrent MCP/channel/plugin writes are not merge-patch deleted during npm install.
