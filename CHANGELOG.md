...
- CLI/channels: overlay only intended plugin-install mutations when `channels login|logout`, `channels capabilities`, `channels resolve`, and `directory` persist after on-demand catalog installs, so concurrent config keys written during npm/prompt I/O are no longer wiped.
