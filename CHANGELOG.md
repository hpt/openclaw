...
- Commands/export: keep `/export` and `/export-session` output files inside the agent workspace so authorized senders cannot overwrite host config or credential files.
