...
- Gateway/sessions: abort an in-progress agent run and take the session write lock before Control UI `/compact` rewrites the transcript, and keep the JSONL session header so live turns are not lost and SessionManager can still open the file.
