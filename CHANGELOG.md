...
- Discord/voice: honor `commands.ownerAllowFrom` when marking voice speakers as owners, so Discord `allowFrom` alone cannot grant owner-only tools (`gateway`, `cron`, `nodes`) to non-owners.
