...
- Commands/ACP: require owner access for mutating `/acp` actions on external channels (matching the existing internal `operator.admin` gate), so authorized non-owners cannot spawn or steer host ACP sessions.
