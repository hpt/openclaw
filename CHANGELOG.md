...
- Plugins/uninstall: re-read config after plugin directory deletion before persisting the removal, so concurrent config writes during `fs.rm` are not merge-patch wiped.
