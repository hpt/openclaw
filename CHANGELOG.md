...
- Feishu/dynamic agents: persist new DM agent bindings onto a fresh config snapshot instead of the monitor's register-time config, so creating an agent no longer wipe-patches concurrent keys such as MCP servers, other channels, or other dynamic agents.
