...
- Doctor/cron: re-read `cron/jobs.json` after the legacy-repair confirm prompt so concurrent gateway job adds and last-run updates are not overwritten by the pre-prompt snapshot.
