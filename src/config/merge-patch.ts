 ...
 * Missing keys in `target` become `null` (delete).
 */
...
export function mergeConfigMutationsOntoFresh<T>(params: { fresh: T; baseline: T; next: T }): T {
  const patch = createMergePatch(params.baseline, params.next);
